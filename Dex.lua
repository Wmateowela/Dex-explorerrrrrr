...
--[[
	...
	The main explorer interface
		...
		local function main()
			...
			local dexLoadingFrame = nil
			local function setDexLoading(active, text)
				if active then
					if not dexLoadingFrame then
						dexLoadingFrame = create({
							{1,"Frame",{BackgroundColor3=Color3.new(0.05,0.05,0.1),BackgroundTransparency=0.2,BorderSizePixel=0,Name="LoadingOverlay",Size=UDim2.new(1,0,1,0),ZIndex=100}},
							{2,"UICorner",{CornerRadius=UDim.new(0,5),Parent={1}}},
							{3,"TextLabel",{BackgroundTransparency=1,Font=4,Name="LoadText",Parent={1},Size=UDim2.new(1,0,1,0),Text="⏳ Loading...",TextColor3=Color3.new(1,0.66,0),TextSize=16,ZIndex=101}}
						})
					end
					dexLoadingFrame.LoadText.Text = text or "⏳ Loading..."
					dexLoadingFrame.Parent = Explorer.Window.GuiElems.Content
					dexLoadingFrame.Visible = true
				elseif dexLoadingFrame then
					dexLoadingFrame.Visible = false
				end
			end

			local function formatPropertyValue(val)
				local t = typeof(val)
				if t == "Vector3" then
					return "{" .. string.format("%.3f, %.3f, %.3f", val.X, val.Y, val.Z) .. "}"
				elseif t == "Vector2" then
					return "Vec2{" .. string.format("%.3f, %.3f", val.X, val.Y) .. "}"
				elseif t == "UDim2" then
					return "UDim2{" .. string.format("%.3f, %d, %.3f, %d", val.X.Scale, val.X.Offset, val.Y.Scale, val.Y.Offset) .. "}"
				elseif t == "UDim" then
					return "UDim{" .. string.format("%.3f, %d", val.Scale, val.Offset) .. "}"
				elseif t == "Rect" then
					return "Rect{" .. string.format("%.3f, %.3f, %.3f, %.3f", val.Min.X, val.Min.Y, val.Max.X, val.Max.Y) .. "}"
				elseif t == "CFrame" then
					local x, y, z, r1, r2, r3, r4, r5, r6, r7, r8, r9 = val:GetComponents()
					return "{" .. string.format("%.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f", x,y,z, r1,r2,r3,r4,r5,r6,r7,r8,r9) .. "}"
				elseif t == "Color3" then
					return "#" .. val:ToHex()
				elseif t == "ColorSequence" then
					local keys = {}
					for _, kp in ipairs(val.Keypoints) do
						table.insert(keys, string.format("%.3f #%s", kp.Time, kp.Value:ToHex()))
					end
					return "ColorSequence{" .. table.concat(keys, ", ") .. "}"
				elseif t == "NumberSequence" then
					local keys = {}
					for _, kp in ipairs(val.Keypoints) do
						table.insert(keys, string.format("%.3f %.3f %.3f", kp.Time, kp.Value, kp.Envelope))
					end
					return "NumberSequence{" .. table.concat(keys, ", ") .. "}"
				elseif t == "NumberRange" then
					return "NumberRange{" .. string.format("%.3f, %.3f", val.Min, val.Max) .. "}"
				elseif t == "BrickColor" then
					return "BrickColor[" .. val.Name .. "]"
				elseif t == "PhysicalProperties" then
					return "PhysicalProperties{" .. string.format("%.3f, %.3f, %.3f, %.3f, %.3f", val.Density, val.Friction, val.Elasticity, val.FrictionWeight, val.ElasticityWeight) .. "}"
				elseif t == "Faces" then
					local tFaces = {}
					for _, enum in pairs(Enum.NormalId:GetEnumItems()) do
						if val[enum.Name] then table.insert(tFaces, enum.Name) end
					end
					return "Faces[" .. table.concat(tFaces, ", ") .. "]"
				elseif t == "Axes" then
					local tAxes = {}
					for _, enum in pairs(Enum.Axis:GetEnumItems()) do
						if val[enum.Name] then table.insert(tAxes, enum.Name) end
					end
					return "Axes[" .. table.concat(tAxes, ", ") .. "]"
				elseif t == "Instance" then
					return "Inst:" .. val:GetDebugId()
			...
			local function serializeProps(inst, indent)
				indent = indent or ""
				local className = inst.ClassName
				local lines = {}
				table.insert(lines, indent .. "ClassName = " .. className)
				table.insert(lines, indent .. "DexID = " .. inst:GetDebugId())
				
				local apiClass = API.Classes[className]
				local current = apiClass
				local seen = {ClassName = true, Name = true, Parent = true}
				
				while current do
					for _, prop in pairs(current.Properties) do
						local propName = prop.Name
						local tags = prop.Tags
						if not seen[propName] and not tags.Hidden and not tags.Deprecated then
							seen[propName] = true
							pcall(function()
								local val = inst[propName]
								if val ~= nil then
									if propName == "Source" and type(val) == "string" then
										if val == "" and type(env.decompile) == "function" and (inst:IsA("LocalScript") or inst:IsA("ModuleScript")) then
											pcall(function() val = env.decompile(inst) or val end)
										end
										table.insert(lines, indent .. propName .. " = [[DEX_CODE_START]]")
										for sLine in string.gmatch(val, "[^\r\n]+") do
											table.insert(lines, sLine)
										end
										table.insert(lines, "[[DEX_CODE_END]]")
									else
										table.insert(lines, indent .. propName .. " = " .. formatPropertyValue(val))
									end
								end
							end)
						end
			...
			local function serializeDepth1(inst)
				local lines = {serializeProps(inst)}
				local children = {}
				pcall(function() children = inst:GetChildren() end)
				for _, child in pairs(children) do
					table.insert(lines, "  (" .. child.Name .. ") {")
					table.insert(lines, "    ClassName = " .. child.ClassName)
					table.insert(lines, "  }")
				end
				return table.concat(lines, "\n")
			end

			local function serializeChildrenNames(inst)
				local lines = {"[" .. inst.Name .. "] Children:"}
				local children = {}
				pcall(function() children = inst:GetChildren() end)
				for _, child in pairs(children) do
			...
			local function serializeAllNamesAsync(inst, callback)
				setDexLoading(true, "⏳ Calculating Size...")
				task.spawn(function()
					local totalDescendants = 0
					pcall(function() totalDescendants = #inst:GetDescendants() end)
					local totalItems = totalDescendants + 1
					
					local lines = {}
					local count = 0
					local lastYield = os.clock()
					
					local function processNode(node, indent, isRoot)
						count = count + 1
						if count % 20 == 0 and (os.clock() - lastYield) > 0.05 then
							local pct = math.min(100, math.floor((count / totalItems) * 100))
							setDexLoading(true, "⏳ Scraping... " .. pct .. "% (" .. count .. "/" .. totalItems .. ")")
							task.wait()
							lastYield = os.clock()
						end
						local wrap = isRoot and ("[" .. node.Name .. "]") or ("(" .. node.Name .. ")")
						table.insert(lines, indent .. wrap .. " [" .. node.ClassName .. "]")
						local children = {}
						pcall(function() children = node:GetChildren() end)
						for _, child in pairs(children) do
							processNode(child, indent .. "  ", false)
						end
					end
					processNode(inst, "", true)
					setDexLoading(false)
					callback(table.concat(lines, "\n"))
				end)
			end

			local function serializeAllAsync(inst, callback)
				setDexLoading(true, "⏳ Calculating Size...")
				task.spawn(function()
					local totalDescendants = 0
					pcall(function() totalDescendants = #inst:GetDescendants() end)
					...
					local function processNode(node, indent, isRoot)
						count = count + 1
						if count % 10 == 0 and (os.clock() - lastYield) > 0.05 then
							local pct = math.min(100, math.floor((count / totalItems) * 100))
							setDexLoading(true, "⏳ Scraping... " .. pct .. "% (" .. count .. "/" .. totalItems .. ")")
							task.wait()
							lastYield = os.clock()
						...
						local wrapStart = isRoot and "[" or "("
						local wrapEnd = isRoot and "]" or ")"
						
						table.insert(lines, indent .. wrapStart .. node.Name .. wrapEnd .. " {")
						table.insert(lines, serializeProps(node, indent .. "  "))
						table.insert(lines, indent .. "  .") -- Dot separator
			...
			Explorer.ShowRightClick = function(MousePos)
				...
				if env.setclipboard then
					...
					context:AddRegistered("COPY_PROPS", #sList ~= 1)
					context:AddRegistered("COPY_CHILDREN_NAMES", #sList ~= 1)
					context:AddRegistered("COPY_ALL_CHILDREN_NAMES", #sList ~= 1)
					context:AddRegistered("COPY_SURFACE_CHILDREN", #sList ~= 1)
					context:AddRegistered("COPY_ALL_PROPS", #sList ~= 1)
				end
			...
			Explorer.InitRightClick = function()
				...
				context:Register("COPY_PROPS",{Name = "Copy Properties", IconMap = Explorer.MiscIcons, Icon = "Copy", OnClick = function()
					local sList = selection.List
					if sList[1] then
						env.setclipboard(serializeProps(sList[1].Obj))
					end
				end})

				context:Register("COPY_CHILDREN_NAMES",{Name = "Copy Children Names", IconMap = Explorer.MiscIcons, Icon = "Copy", OnClick = function()
					local sList = selection.List
					if sList[1] then
						env.setclipboard(serializeChildrenNames(sList[1].Obj))
					end
				end})

				context:Register("COPY_ALL_CHILDREN_NAMES",{Name = "Copy All Children Names", IconMap = Explorer.MiscIcons, Icon = "Copy", OnClick = function()
					local sList = selection.List
					if sList[1] then
						serializeAllNamesAsync(sList[1].Obj, function(text)
							env.setclipboard(text)
						end)
					end
				end})

				context:Register("COPY_SURFACE_CHILDREN",{Name = "Copy Surface Children", IconMap = Explorer.MiscIcons, Icon = "Copy", OnClick = function()
					local sList = selection.List
					if sList[1] then
						env.setclipboard(serializeDepth1(sList[1].Obj))
					end
				end})

				context:Register("COPY_ALL_PROPS",{Name = "Copy All Properties (Recursive)", IconMap = Explorer.MiscIcons, Icon = "Copy", OnClick = function()
					local sList = selection.List
					if sList[1] then
						serializeAllAsync(sList[1].Obj, function(text)
							env.setclipboard(text)
						end)
					end
				end})
...
--[[
	...
	PartManipulator = function()
--[[
	Part Manipulator App Module
	
	Handles Move, Scale, and Rotate handles for selected parts.
]]
		...
		local function main()
			...
			local moveHandles, scaleHandles, rotateHandles, selectionBox
			...
			local function clearHandles()
				...
				if scaleHandles then scaleHandles:Destroy(); scaleHandles = nil end
				if rotateHandles then rotateHandles:Destroy(); rotateHandles = nil end
				if selectionBox then selectionBox:Destroy(); selectionBox = nil end
			end

			function PartManipulator.UpdateHandles()
				if not PartManipulator.Active then clearHandles(); return end
				local sList = Explorer.Selection.List
				local target = sList[1] and sList[1].Obj
				...
				local canPivot = target:IsA("PVInstance")
				if not canPivot and not target:IsA("Folder") then
					clearHandles()
					return
				end
				...
				if not selectionBox then
					...
					selectionBox.Parent = Main.GuiHolder
				end
				selectionBox.Adornee = target

				local function getPivot()
					return canPivot and target:GetPivot() or part.CFrame
				end
				...
				local function pivotTo(cf)
					if canPivot then
						...
						local oldPivot = part.CFrame
						local delta = cf * oldPivot:Inverse()
						for _, v in ipairs(target:GetDescendants()) do
							if v:IsA("BasePart") then
								v.CFrame = delta * v.CFrame
							end
						end
					end
				end
				...
				if currentMode == "Move" then
					if not moveHandles then
						clearHandles()
						moveHandles = Instance.new("Handles")
						moveHandles.Adornee = part
						moveHandles.Style = Enum.HandlesStyle.Movement
						moveHandles.Color3 = Color3.fromRGB(0, 170, 255)
						moveHandles.Parent = Main.GuiHolder
						moveHandles.MouseButton1Down:Connect(function()
							initialCFrame = getPivot()
							pushUndo({Type = "Transform", Part = target, OldCFrame = initialCFrame, OldSize = (target:IsA("BasePart") and target.Size or Vector3.new())})
						end)
						moveHandles.MouseDrag:Connect(function(face, distance)
							local normal = Vector3.FromNormalId(face)
							pivotTo(initialCFrame * CFrame.new(normal * distance))
						end)
					end
				elseif currentMode == "Scale" then
					if not target:IsA("BasePart") then clearHandles(); return end -- Scale only bits
					if not scaleHandles then
						clearHandles()
						scaleHandles = Instance.new("Handles")
						scaleHandles.Adornee = part
						scaleHandles.Style = Enum.HandlesStyle.Resize
						scaleHandles.Color3 = Color3.fromRGB(0, 255, 100)
						scaleHandles.Parent = Main.GuiHolder
						scaleHandles.MouseButton1Down:Connect(function()
							initialCFrame = target.CFrame
							initialSize = target.Size
							pushUndo({Type = "Transform", Part = target, OldCFrame = initialCFrame, OldSize = initialSize})
						end)
						scaleHandles.MouseDrag:Connect(function(face, distance)
							...
							local newSize = initialSize + (axis * distance)
							newSize = Vector3.new(math.max(newSize.X, 0.2), math.max(newSize.Y, 0.2), math.max(newSize.Z, 0.2))
							target.Size = newSize
							target.CFrame = initialCFrame * CFrame.new(normal * (distance / 2))
						end)
					end
				elseif currentMode == "Rotate" then
					if not rotateHandles then
						clearHandles()
						rotateHandles = Instance.new("ArcHandles")
						rotateHandles.Adornee = part
						rotateHandles.Color3 = Color3.fromRGB(245, 180, 0)
						rotateHandles.Parent = Main.GuiHolder
						rotateHandles.MouseButton1Down:Connect(function()
							initialCFrame = getPivot()
							pushUndo({Type = "Transform", Part = target, OldCFrame = initialCFrame, OldSize = (target:IsA("BasePart") and target.Size or Vector3.new())})
						end)
						rotateHandles.MouseDrag:Connect(function(axis, relativeAngle)
							local rotation = CFrame.fromAxisAngle(Vector3.FromAxis(axis), relativeAngle)
							pivotTo(initialCFrame * rotation)
						end)
					end
				else
					clearHandles()
				...
				if selectionBox then selectionBox.Adornee = target end
			end

			function PartManipulator.SetMode(mode)
				currentMode = mode
				for name, btn in pairs(modeButtons) do
					if btn:IsA("TextButton") then
						btn.BackgroundColor3 = (name == mode) and Settings.Theme.ListSelection or Settings.Theme.Button
					end
				end
				PartManipulator.UpdateHandles()
			end

			function PartManipulator.Init()
				window = Lib.Window.new()
				PartManipulator.Window = window
				window:SetTitle("Part Manipulator")
				window.GuiElems.Main.Size = UDim2.new(0, 200, 0, 400)
				window.GuiElems.Line.Position = UDim2.new(0,0,0,22)

				local content = window.GuiElems.Content
				local scroller = createSimple("ScrollingFrame", {
					Size = UDim2.new(1, 0, 1, 0),
					BackgroundTransparency = 1,
				...
				local function createBtn(name, icon, callback)
					...
					btn.MouseButton1Click:Connect(callback or function() PartManipulator.SetMode(name) end)
					if not callback then modeButtons[name] = btn end
					return btn
				end
				...
				createBtn("Duplicate", "📋", function()
					local sList = Explorer.Selection.List
					local target = sList[1] and sList[1].Obj
					if target then
						local clone = target:Clone()
						if target:IsA("BasePart") then
							clone.CFrame = target.CFrame * CFrame.new(0, target.Size.Y + 0.5, 0)
						elseif target:IsA("Model") then
							clone:PivotTo(target:GetPivot() * CFrame.new(0, 5, 0))
						end
						clone.Parent = target.Parent
						pushUndo({Type = "Create", Part = clone})
					end
				end)

				createBtn("Anchor", "⚓", function()
					local sList = Explorer.Selection.List
					local target = sList[1] and sList[1].Obj
					if target then
						if target:IsA("BasePart") then
							pushUndo({Type = "Anchor", Part = target, OldAnchored = target.Anchored})
							target.Anchored = not target.Anchored
						else
							local firstPart = target:FindFirstChildWhichIsA("BasePart", true)
							if firstPart then
								local newState = not firstPart.Anchored
								for _, v in ipairs(target:GetDescendants()) do
									if v:IsA("BasePart") then
										pushUndo({Type = "Anchor", Part = v, OldAnchored = v.Anchored})
										v.Anchored = newState
									end
								end
							end
				...
				createBtn("Delete", "🗑️", function()
					local sList = Explorer.Selection.List
					local target = sList[1] and sList[1].Obj
					if target then
						Main.PushUndo({Type = "Delete", Obj = target, Parent = target.Parent})
						target.Parent = nil
					end
				end)

				local undoBtn = createBtn("Undo", "↩️", undo)
				undoBtn.BackgroundColor3 = Color3.fromRGB(80, 40, 40)

				local shortcutBtn = createBtn("Shortcuts: ON", "⌨️", function() end)
				shortcutBtn.BackgroundColor3 = Color3.fromRGB(40, 80, 40)
				shortcutBtn.MouseButton1Click:Connect(function()
					shortcutsEnabled = not shortcutsEnabled
					shortcutBtn.Text = "⌨️ Shortcuts: " .. (shortcutsEnabled and "ON" or "OFF")
					shortcutBtn.BackgroundColor3 = shortcutsEnabled and Color3.fromRGB(40, 80, 40) or Color3.fromRGB(60, 60, 80)
				end)

				PartManipulator.SetMode("Select")

				Explorer.Selection.Changed:Connect(function()
					if PartManipulator.Active then PartManipulator.UpdateHandles() end
				end)

				window.OnActivate:Connect(function()
					PartManipulator.Active = true
					PartManipulator.UpdateHandles()
				end)
				window.OnDeactivate:Connect(function()
					PartManipulator.Active = false
					clearHandles()
				end)

				-- Mouse Selection Connection
				Main.Mouse.Button1Down:Connect(function()
					if not PartManipulator.Active or currentMode ~= "Select" then return end
					local target = Main.Mouse.Target
					if target and nodes[target] then
						Explorer.Selection:Set(nodes[target])
						Explorer.ViewNode(nodes[target])
					end
				end)
				...
				service.UserInputService.InputBegan:Connect(function(input, processed)
					...
					if input.KeyCode == Enum.KeyCode.Z and (service.UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) or service.UserInputService:IsKeyDown(Enum.KeyCode.RightControl)) then
						undo()
					elseif shortcutsEnabled then
						if input.KeyCode == Enum.KeyCode.One then PartManipulator.SetMode("Select")
						elseif input.KeyCode == Enum.KeyCode.Two then PartManipulator.SetMode("Move")
						elseif input.KeyCode == Enum.KeyCode.Three then PartManipulator.SetMode("Rotate")
						elseif input.KeyCode == Enum.KeyCode.Four then PartManipulator.SetMode("Scale")
						elseif input.KeyCode == Enum.KeyCode.Delete or input.KeyCode == Enum.KeyCode.Backspace then
							local sList = Explorer.Selection.List
							local part = sList[1] and sList[1].Obj
							if part and part:IsA("BasePart") then
								local data = {
									Name = part.Name, Size = part.Size, CFrame = part.CFrame,
									Anchored = part.Anchored, CanCollide = part.CanCollide,
									Color = part.Color, Material = part.Material, Transparency = part.Transparency
								}
								pushUndo({Type = "Delete", Data = data, Parent = part.Parent})
								part:Destroy()
							end
						elseif input.KeyCode == Enum.KeyCode.D and (service.UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) or service.UserInputService:IsKeyDown(Enum.KeyCode.RightControl)) then
							local sList = Explorer.Selection.List
							local target = sList[1] and sList[1].Obj
							if target then
								local clone = target:Clone()
...
--[[
	...
	The AI Codebase Explorer & Agent Chatbot interface
]]

		-- Common Locals
		local Main,Lib,Apps,Settings
		local Explorer, Properties, ScriptViewer, Notebook, AIAssistant
		...
		local function initAfterMain()
			...
			ScriptViewer = Apps.ScriptViewer
			Notebook = Apps.Notebook
			AIAssistant = Apps.AIAssistant
		end

		local function main()
			local AIAssistant = {}
			local window, scrollFrame, inputTextBox, sendButton, settingsBtn, settingsPanel, statusLabel

			local apiConfig = {
				baseURL = "https://openrouter.ai/api/v1",
				apiKey = "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",
			...
			local function getObjectFromPath(pathStr)
				if type(pathStr) ~= "string" or pathStr == "" or pathStr == "game" then return game end
				local cleanPath = pathStr:gsub("^game%.", ""):gsub("^workspace", "Workspace")
				if cleanPath == "Workspace" or cleanPath == "workspace" then return workspace end

				local parts = {}
				for part in cleanPath:gmatch("[^%.%[%]'\"]+") do
					if part ~= "" and part ~= "GetChildren()" and part ~= "GetService" then
				...
				for _, part in ipairs(parts) do
					...
					if not found then
						for _, child in ipairs(current:GetChildren()) do
							if child.Name:lower() == part:lower() then
								found = child
								break
							end
						end
			...
			local toolsSchema = {
				{
					...
					["function"] = {
						name = "glob",
						description = "Find instances by name pattern or class name across game tree.",
						parameters = {
							type = "object",
							properties = {
								pattern = {type = "string", description = "Name or Class search pattern (e.g. Remote*, Part, Spawn*)"},
								parentPath = {type = "string", description = "Parent path to search inside (default 'game')"}
							},
							required = {"pattern"}
						}
					}
				},
				{
					...
					["function"] = {
						name = "grep",
						description = "Search script source code or property values across the game for a pattern.",
						parameters = {
							type = "object",
							properties = {
								pattern = {type = "string", description = "Text or pattern to search for"},
								searchType = {type = "string", description = "Search type: 'script', 'property', or 'all'"}
							},
							required = {"pattern"}
						}
					}
				},
				{
					...
					["function"] = {
						name = "read",
						description = "Inspect an instance's properties, children list, and source code if it is a script.",
						parameters = {
							type = "object",
							properties = {
								instancePath = {type = "string", description = "Path to instance (e.g. workspace.Part1 or ReplicatedStorage.Module)"}
							},
				...
				},
				{
					...
					["function"] = {
						name = "execute_lua",
						description = "Run arbitrary Luau code in executor context. Full access to Roblox API & executor.",
						parameters = {
							type = "object",
							properties = {
				...
				},
				{
					...
					["function"] = {
						name = "press_key",
						description = "Simulate keypress or character direction input (e.g. W, A, S, D, Space).",
						parameters = {
							type = "object",
							properties = {
								key = {type = "string", description = "Key name (W, A, S, D, Space, etc.)"},
				...
				},
				{
					...
					["function"] = {
						name = "move_to",
						description = "Teleport or move local player to position Vector3 (e.g. '0, 10, 0') or instance path.",
						parameters = {
							type = "object",
							properties = {
								target = {type = "string", description = "Position coordinates 'X, Y, Z' or instance path"}
							},
							required = {"target"}
						}
					}
				},
				{
					...
					["function"] = {
						name = "set_property",
						description = "Set property value of an instance in game.",
						parameters = {
							type = "object",
							properties = {
								instancePath = {type = "string", description = "Instance path"},
								propertyName = {type = "string", description = "Property name"},
								value = {type = "string", description = "Value to set (auto-converted)"}
							},
							required = {"instancePath", "propertyName", "value"}
						}
					}
				},
				{
					...
					["function"] = {
						name = "fire_remote",
						description = "Fire RemoteEvent/RemoteFunction/BindableEvent/ClickDetector/ProximityPrompt.",
						parameters = {
							type = "object",
							properties = {
								targetPath = {type = "string", description = "Target instance path"},
								action = {type = "string", description = "Action: FireServer, InvokeServer, Fire, Click, Prompt"}
							},
			...
			local function executeToolCall(toolName, args)
				...
				elseif toolName == "execute_lua" then
			...
			local function autoScroll()
				task.wait(0.05)
				if scrollFrame then
					scrollFrame.CanvasPosition = Vector2.new(0, scrollFrame.UIListLayout.AbsoluteContentSize.Y + 200)
				end
			end
			...
			local function addUserMessage(text)
				...
				local bubble = createSimple("Frame", {
					Size = UDim2.new(0.85, 0, 0, 0),
					AutomaticSize = Enum.AutomaticSize.Y,
					Position = UDim2.new(0.15, 0, 0, 0),
					BackgroundColor3 = Settings.Theme.ListSelection or Color3.fromRGB(11, 90, 175),
					BorderSizePixel = 0,
					Parent = msgFrame
				})
				createSimple("UICorner", {CornerRadius = UDim.new(0, 6), Parent = bubble})
				createSimple("UIPadding", {PaddingTop = UDim.new(0, 6), PaddingBottom = UDim.new(0, 6), PaddingLeft = UDim.new(0, 8), PaddingRight = UDim.new(0, 8), Parent = bubble})
			...
			local function addToolCard(toolName, args)
				local card = createSimple("Frame", {
					Size = UDim2.new(1, -10, 0, 0),
					AutomaticSize = Enum.AutomaticSize.Y,
					BackgroundColor3 = Color3.fromRGB(30, 32, 38),
					BorderColor3 = Color3.fromRGB(60, 64, 76),
					Parent = scrollFrame
				})
				createSimple("UICorner", {CornerRadius = UDim.new(0, 5), Parent = card})
				createSimple("UIPadding", {PaddingTop = UDim.new(0, 5), PaddingBottom = UDim.new(0, 5), PaddingLeft = UDim.new(0, 8), PaddingRight = UDim.new(0, 8), Parent = card})

				local argsText = service.HttpService:JSONEncode(args or {})
				createSimple("TextLabel", {
					Size = UDim2.new(1, 0, 0, 0),
					AutomaticSize = Enum.AutomaticSize.Y,
					BackgroundTransparency = 1,
					Text = "🔧 <b>Executing Tool:</b> <font color=\"#4DA6FF\">" .. toolName .. "</font>\n<font color=\"#AAAAAA\">" .. argsText .. "</font>",
			...
			local function addToolResultCard(toolName, resultStr)
				local card = createSimple("Frame", {
					Size = UDim2.new(1, -10, 0, 0),
					AutomaticSize = Enum.AutomaticSize.Y,
					BackgroundColor3 = Color3.fromRGB(25, 28, 32),
					BorderColor3 = Color3.fromRGB(50, 54, 62),
					Parent = scrollFrame
				})
				createSimple("UICorner", {CornerRadius = UDim.new(0, 5), Parent = card})
				createSimple("UIPadding", {PaddingTop = UDim.new(0, 5), PaddingBottom = UDim.new(0, 5), PaddingLeft = UDim.new(0, 8), PaddingRight = UDim.new(0, 8), Parent = card})

				local snippet = tostring(resultStr):sub(1, 500)
				createSimple("TextLabel", {
					Size = UDim2.new(1, 0, 0, 0),
					AutomaticSize = Enum.AutomaticSize.Y,
					BackgroundTransparency = 1,
					Text = "⚡ <b>Tool Result (" .. toolName .. "):</b>\n<font color=\"#ADF195\">" .. snippet .. "</font>",
			...
			local function addAssistantMessage(rawText)
				...
				local bubble = createSimple("Frame", {
					...
					BackgroundColor3 = Settings.Theme.Main2 or Color3.fromRGB(45, 45, 45),
					BorderSizePixel = 0,
					Parent = msgFrame
				})
				createSimple("UICorner", {CornerRadius = UDim.new(0, 6), Parent = bubble})
				...
				local layout = Instance.new("UIListLayout", bubble)
			...
			local function handleUserSubmit()
				if isProcessing then return end
				local userText = inputTextBox.Text
				if userText:gsub("%s+", "") == "" then return end
...
5. HUMAN LANGUAGE: Explain what actions you took step-by-step in natural, friendly human language (Urdu / Roman Urdu / English as requested by user). Use bold (**text**), italic (*text*), and code formatting.]]
				...
				isProcessing = true
				statusLabel.Text = "⏳ AI is thinking & exploring codebase..."

				task.spawn(function()
					local req = env.request or request or http_request or (syn and syn.request) or (http and http.request)
					if not req then
						addAssistantMessage("Error: HTTP Request function not supported by executor.")
					...
					while iteration < maxIterations do
						...
						local success, err = pcall(function()
							response = req({
								...
								Body = service.HttpService:JSONEncode(payload)
							})
						end)

						if not success or not response then
							addAssistantMessage("Error connecting to AI API: " .. tostring(err))
							break
						end

						if response.StatusCode == 200 or response.Success then
							local data = nil
							pcall(function() data = service.HttpService:JSONDecode(response.Body) end)
							
							if type(data) == "table" then
								if data.error then
									local errStr = type(data.error) == "table" and (data.error.message or service.HttpService:JSONEncode(data.error)) or tostring(data.error)
									addAssistantMessage("API Error from Server: " .. errStr)
									break
								end
								...
								if data.choices and data.choices[1] then
									local c = data.choices[1]
									if c.message then
										msg = c.message
										replyContent = c.message.content
									...
									for _, tc in ipairs(toolCalls) do
										...
										pcall(function()
											if type(fn.arguments) == "string" then
												args = service.HttpService:JSONDecode(fn.arguments)
								...
								addAssistantMessage("Error: Could not decode JSON API response.\nRaw Response: " .. tostring(response.Body:sub(1, 300)))
								break
							end
						else
							addAssistantMessage("API Error (" .. tostring(response.StatusCode) .. "): " .. tostring(response.Body))
							break
						end
					end
			...
			function AIAssistant.Init()
				...
				local headerBar = createSimple("Frame", {
					Size = UDim2.new(1, 0, 0, 28),
					BackgroundColor3 = Settings.Theme.Main1 or Color3.fromRGB(52, 52, 52),
					BorderSizePixel = 0,
					Parent = content
				})
				...
				settingsBtn = createSimple("TextButton", {
				...
				createSimple("UICorner", {CornerRadius = UDim.new(0, 4), Parent = settingsBtn})
				...
				createSimple("UICorner", {CornerRadius = UDim.new(0, 4), Parent = clearBtn})
				...
				local inputPanel = createSimple("Frame", {
					Size = UDim2.new(1, -6, 0, 36),
					Position = UDim2.new(0, 3, 1, -40),
					BackgroundColor3 = Settings.Theme.Main1 or Color3.fromRGB(52, 52, 52),
					BorderSizePixel = 0,
					Parent = content
				})
				createSimple("UICorner", {CornerRadius = UDim.new(0, 5), Parent = inputPanel})

				inputTextBox = createSimple("TextBox", {
					Size = UDim2.new(1, -45, 1, -6),
					Position = UDim2.new(0, 6, 0, 3),
					BackgroundColor3 = Settings.Theme.TextBox or Color3.fromRGB(38, 38, 38),
					BorderSizePixel = 0,
					PlaceholderText = "Ask AI to explore, move, press keys, fix script...",
				...
				createSimple("UICorner", {CornerRadius = UDim.new(0, 4), Parent = inputTextBox})

				sendButton = createSimple("TextButton", {
				...
				createSimple("UICorner", {CornerRadius = UDim.new(0, 4), Parent = sendButton})

				sendButton.MouseButton1Click:Connect(handleUserSubmit)
				inputTextBox.FocusLost:Connect(function(enterPressed)
					if enterPressed then handleUserSubmit() end
				end)

				settingsPanel = createSimple("Frame", {
				...
				createSimple("UICorner", {CornerRadius = UDim.new(0, 6), Parent = settingsPanel})

				createSimple("TextLabel", {
					Size = UDim2.new(1, 0, 0, 30),
					Position = UDim2.new(0, 10, 0, 5),
					BackgroundTransparency = 1,
					Text = "⚙️ AI Settings Configuration",
					TextColor3 = Color3.fromRGB(255, 255, 255),
					Font = 4,
					TextSize = 15,
					TextXAlignment = Enum.TextXAlignment.Left,
				...
				local function createField(labelStr, defaultVal, yPos)
					createSimple("TextLabel", {
					...
					local box = createSimple("TextBox", {
						...
						Position = UDim2.new(0, 10, 0, yPos + 18),
				...
				local urlBox = createField("Base URL:", apiConfig.baseURL, 40)
				local keyBox = createField("API Key:", apiConfig.apiKey, 90)
				local modelBox = createField("Model Name:", apiConfig.model, 140)

				local saveSettingsBtn = createSimple("TextButton", {
				...
				createSimple("UICorner", {CornerRadius = UDim.new(0, 4), Parent = saveSettingsBtn})

				local resetSettingsBtn = createSimple("TextButton", {
					Size = UDim2.new(0.45, 0, 0, 28),
					Position = UDim2.new(0.5, 5, 0, 200),
				...
				createSimple("UICorner", {CornerRadius = UDim.new(0, 4), Parent = resetSettingsBtn})

				saveSettingsBtn.MouseButton1Click:Connect(function()
					apiConfig.baseURL = urlBox.Text
					apiConfig.apiKey = keyBox.Text
					apiConfig.model = modelBox.Text
					settingsPanel.Visible = false
				end)

				resetSettingsBtn.MouseButton1Click:Connect(function()
					apiConfig.baseURL = "http://localhost:20128/v1"
					apiConfig.apiKey = "sk-4264e73cdc6ea2e6-9f3e1f-a15477ee"
					apiConfig.model = "Test-combo"
					urlBox.Text = apiConfig.baseURL
					keyBox.Text = apiConfig.apiKey
					modelBox.Text = apiConfig.model
				end)
				...
				window.OnActivate:Connect(function()
					AIAssistant.Active = true
				end)
				window.OnDeactivate:Connect(function()
					AIAssistant.Active = false
				end)
			end
...
-- Main vars
local Main, Explorer, Properties, ScriptViewer, DefaultSettings, Notebook, Serializer, Lib, Console, SaveInstance, PartManipulator, AIAssistant
local API, RM
...
Main = (function()
	...
	Main.ModuleList = {"Explorer", "Properties", "ScriptViewer", "Console", "SaveInstance", "PartManipulator", "AIAssistant"}
	Main.Elevated = false
	Main.MissingEnv = {}
	...
	Main.LoadModules = function()
		...
		Notebook = Apps.Notebook
		PartManipulator = Apps.PartManipulator
		AIAssistant = Apps.AIAssistant
	...
	Main.CreateMainGui = function()
		...
		Main.CreateApp({Name = "Drag Part", IconMap = Main.LargeIcons, Icon = "Properties", Window = PartManipulator.Window})

		Main.CreateApp({Name = "AI", IconMap = Main.LargeIcons, Icon = "Watcher", Window = AIAssistant.Window})
